...
export const createCompletedSlides = (length: number, firstUnlocked: boolean = true): boolean[] => {
  return [firstUnlocked, ...Array(length - 1).fill(false)];
};

export const COMPLETED_SLIDES = createCompletedSlides(29); // 29 slides

@Component({
  selector: 'app-individual-letters',
  standalone: true,
  imports: [CommonModule, CarouselModule, SharedModule],
  templateUrl: './individual-letters.component.html',
  styleUrl: './individual-letters.component.css'
})
export class IndividualLettersComponent implements OnInit {
  ...
  constructor(
    ...
    // private INDIVIDUAL_LETTERS_SERVICE: IndividualLettersService,
 ...
 */
  GETALLLETTERS() {
    this.GENERIC_SERVICE.GET_ALL("IndividualLetters/all").subscribe({
      next: (response:any) => {
        ...
        console.log("<------Slider Data--------->",this.SLIDER_DATA)
  ...
  TIP_LIST: { ACTUAL_TEXT: string, EXPECTED_TEXT: string, IS_CORRECT: boolean }[] = [];
  TIPLIST() {
    this.TIP_LIST = [
      { ACTUAL_TEXT: this.SELECTED_SLIDE.urdu, EXPECTED_TEXT: this.SELECTED_SLIDE.urdu, IS_CORRECT: true },   // First Tip already green
      { ACTUAL_TEXT: this.SELECTED_SLIDE.urdu, EXPECTED_TEXT: this.SELECTED_SLIDE.urdu, IS_CORRECT: false }   // Second Tip red initially
    ];
  }
